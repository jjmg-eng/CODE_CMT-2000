...
rlSocket       sock("172.17.21.51",502,1);
rlSharedMemory shm("/srv/automation/shm/modbus.shm",46);
...
int main(int ac, char **av)
{
  ...
  while(1)
  {
    ...
    ret = modbusCycle(offset,13,3,1140,6);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,13,3,1227,1);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,13,3,1247,1);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,13,3,1267,1);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,13,3,1237,1);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,13,3,1257,1);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,13,3,1277,1);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,13,3,3194,4);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,13,3,3200,2);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,13,3,3222,2);
    if(ret>0) offset += ret; else continue;
  }
